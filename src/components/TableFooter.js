...
function TableFooter (props) {
  ...
  return (
    <StyledTableFooter>
      ...
      <Select onChange={handleSelectChange} aria-label='select pages' className='select'>
      ...
      <div className='buttons'>
        <IconButton disabled={currentPage === 0} onClick={handlePrevPage}>
          <LeftArrow aria-label='previous page' data-testid='previous page' />
        </IconButton>

        <IconButton disabled={currentPage >= Math.ceil(count / rowsPerPage) - 1} onClick={handleNextPage}>
          <RightArrow aria-label='next page' onClick={handleNextPage} data-testid='next page' />
        </IconButton>
      </div>
    </StyledTableFooter>
  );
